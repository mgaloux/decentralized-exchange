...
async function main() {
    ...
    for (let i = 1; i <= 10; i++) {
        transaction = await exchange.connect(user1).makeOrder(mETH.address, tokens(10 * i), ZeqToken.address, tokens(10), { gasLimit: 30000000 })
        await transaction.wait()
        await wait(1)
        console.log(`Made order from user 1 ${user1.address}`)
        transaction = await exchange.connect(user2).makeOrder(ZeqToken.address, tokens(10), mETH.address, tokens(10 * i), { gasLimit: 30000000 })
        await transaction.wait()
        await wait(1)
